...
select avg(no)
  from (select site_id,
	       ...
	       apply_calib(site_id, 'NO', no, corrected_time) as no,
	       data_dict->'no_flag' as no_flag
	  from (select *,
		       correct_instrument_time(site_id, 'DRDAS PC', source, instrument_time) as corrected_time
...
select site_id,
       ...
       corrected_time as time,
       apply_calib(site_id, 'NO', no, corrected_time) as no,
       interpolate_cal(site_id, 'NO', 'zero', corrected_time) as no_zero,
       data_dict->'no_flag' as no_flag
  from (select *,
	       correct_instrument_time(site_id, 'DRDAS PC', source, instrument_time) as corrected_time
