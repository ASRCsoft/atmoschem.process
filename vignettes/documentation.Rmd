...
db = dbConnect(PostgreSQL(), dbname = 'chem2')
```
...
graph %>% export_graph(file_name = 'img/overview.pdf')
knitr::include_graphics('img/overview.pdf')
```
...
dm_atmoschem <- dbGetQuery(db, sQuery)
dm_atmoschemdm <- as.data_model(dm_atmoschem)
...
rsvg::rsvg_pdf(charToRaw(DiagrammeRsvg::export_svg(dm_render_graph(graph))),
               file = 'img/schema.pdf')
knitr::include_graphics('img/schema.pdf')
```
...
Measurements outside the range of realistic values are flagged. Table \@ref(tab:valuefilters) shows a selection of measurement types with their corresponding ranges.
